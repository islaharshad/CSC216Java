...
public class StudentDirectoryTest {
	...
	public void testAddStudent() {
		...
		//Test invalid Student with empty password
		try{
		sd.addStudent("Stu", "Dent", "sdent", "sdent@ncsu.edu", PASSWORD, "", 15);
		} catch (IllegalArgumentException e) {
		assertEquals("Invalid password", e.getMessage());
		}
		
		// Test invalid student with both empty passwords
		try{
			sd.addStudent("Stu", "Dent", "sdent", "sdent@ncsu.edu", "", "", 15);
			} catch (IllegalArgumentException e) {
			assertEquals("Invalid password", e.getMessage());
			}
		...
		try{
			sd.addStudent("Stu", "Dent", "sdent", "sdent@ncsu.edu", null, "", 15);
			} catch (IllegalArgumentException e) {
			assertEquals("Invalid password", e.getMessage());
			}
		
		// Test invalid student with unmatched password
		try  {
			sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, "pwdd", MAX_CREDITS);
		} catch (IllegalArgumentException e){
			assertEquals("Passwords do not match", e.getMessage());
		}
		//Test invalid Student with null repeated password
		try  {
	...
	public void testGetStudentById() {
		...
		assertEquals(s1, sd.getStudentById(ID));
		
		assertEquals(null, sd.getStudentById(""));
		assertEquals(null, sd.getStudentById(null));
	}
	/**
