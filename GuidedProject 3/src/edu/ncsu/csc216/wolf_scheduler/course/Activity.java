...
//	private static int UPPER_HOUR = 60;
	...
	/**
	 * Tests whether each meeting day is the same. If not the same,
	 * returns nothing. Else checks whether the start time of
	 * one activity is less than or equal to the beginning of 
	 * another activity. It also checks whether the end time of 
	 * the latter activity is greater or equal to the start time of the 
	 * formal activity. Thus, if the above conditions are true,there
	 * is an overlap. As a result, the method
	 * throws a Conflict exception
	 * @exception ConflictException checked exception thrown if 
	...
	public void checkConflict(Activity possibleConflictingActivity) throws ConflictException {
		...
		char[] day1 = this.meetingDays.toCharArray();
		char[] day2 = possibleConflictingActivity.meetingDays.toCharArray();
		...
		for (int i = 0; i < this.meetingDays.length(); i++) {
			...
			if ((sameDay) && (this.getStartTime() <= possibleConflictingActivity.getEndTime()) && 
