...
public class RegistrationManagerTest {
	...
	/**
	 * Tests RegistrationManager.login(), RegistrationManager.getStudentDirectory(), and RegistrationManager,getCourseCatalog()
	 */
	@Test
	public void testLoginAndGetStudentDirectoryAndGetCourseCatalog() {
		manager = RegistrationManager.getInstance();
		try{
	...
	/**
	 * Tests RegistrationManager.getCurrectUser()
	 */
	@Test
	...
	/**
	 * Tests Faculty functionality in Registration Manager
	 */
	@Test
	public void testFaculty() {
		manager.login("registrar", "Regi5tr@r");
		
		//Adding Courses to FacultySchedule
		Course c1 = new Course("CSC945", "Sudies in NumLock", "003", 3, null, 25, "MW");
		Course c2 = new Course("CSC946", "Sudies in the Spacebar", "002", 3, null, 25, "T");
		Course c3 = new Course("CSC940", "Studies in Copying and Pasting", "002", 4, null, 25, "H");
		
		
		Faculty faculty = new Faculty("Jon", "Johnson", "jjohnson", "jjohnson@ncsu.edu", "pw", 2);
		
		assertTrue(manager.addFacultyToCourse(c1, faculty));
		assertEquals(1, faculty.getSchedule().getNumScheduledCourses());
		assertTrue(manager.addFacultyToCourse(c2, faculty));
		assertEquals(2, faculty.getSchedule().getNumScheduledCourses());
		
		//Removing Courses from FacultySchedule
		assertTrue(manager.removeFacultyFromCourse(c2, faculty));
		assertEquals(1, faculty.getSchedule().getNumScheduledCourses());
		assertTrue(manager.removeFacultyFromCourse(c1, faculty));
		assertEquals(0, faculty.getSchedule().getNumScheduledCourses());
		
		//Resetting the Schedule
		assertTrue(manager.addFacultyToCourse(c1, faculty));
		assertTrue(manager.addFacultyToCourse(c2, faculty));
		assertTrue(manager.addFacultyToCourse(c3, faculty));
		assertEquals(3, faculty.getSchedule().getNumScheduledCourses());
